...
const PageViews = ({navigation}) => {
  ...
  async function populateData() {
    try {
      ...
      await axios.get(`${apiURL}/jobs/views/${CURRENT_JOB_ID}`).then(res => {
            ...
            {views.map(elem => {
              return (
                <AnalyticsCard
                  ...
                  firstName={elem.author ? elem.author.firstName : 'Anonymous User'}
...
const CallActions = ({navigation}) => {
  ...
  async function populateData() {
    try {
      ...
      await axios.get(`${apiURL}/jobs/calls/${CURRENT_JOB_ID}`).then(res => {
        if (res.data.success === true) {
...
const EmailActions = ({navigation}) => {
  ...
  async function populateData() {
    try {
      ...
      await axios.get(`${apiURL}/jobs/emails/${CURRENT_JOB_ID}`).then(res => {
        if (res.data.success === true) {
...
export default function JobProfileAnalytics({navigation}) {
  ...
  async function populateData() {
    try {
      ...
      await axios
        ...
        .then(res => {
          if (res.data.success === true) {
            return setJobs(res.data.message.reverse());
          } else {
          ...
          return (
            <View key={job.id}>
              ...
              <View style={{paddingHorizontal: 10}}>
                ...
                <Text
                  style={{color: COLORS.white, fontSize: SIZES.text1, flex: 1}}>
                  CATEGORY: {job.category ? job.category.name : 'No Category Added'}
                </Text>
                <Text
                  style={{color: COLORS.white, fontSize: SIZES.text1, flex: 1}}>
                  EMAIL : {job.email}
                ...
                <Text
                  style={{color: COLORS.white, fontSize: SIZES.text1, flex: 1}}>
                  AMOUNT : ${job.billingAmount}
                </Text>
                <Text
                  style={{color: COLORS.white, fontSize: SIZES.text1, flex: 1}}>
                  PAY : {job.wage} {job.wagePeriod}
                </Text>
                {job.wesite ? (
                  <Text
                    style={{
                ...
                <Text
                  ...
                  {/* {{color: COLORS.white, fontSize: SIZES.text1, paddingTop: 10}} */}
                  {job.description}
                </Text>
                {job.website ? (
              ...
              <View key={elem._id}>
                ...
                <View style={{flexDirection: 'column', padding: 5}}>
                  <Tab.Navigator
                    ...
                    <Tab.Screen name="Applied" component={EmailActions} />
                  </Tab.Navigator>
                </View>
              </View>
            );
