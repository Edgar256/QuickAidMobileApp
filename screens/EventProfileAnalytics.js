...
const PageViews = ({navigation}) => {
  ...
  async function populateData() {
    try {
      ...
      await axios
        .get(`${apiURL}/events/views/${CURRENT_EVENT_ID}`)
        .then(res => {
...
const CallActions = ({navigation}) => {
  ...
  async function populateData() {
    try {
      ...
      await axios
        .get(`${apiURL}/events/calls/${CURRENT_EVENT_ID}`)
        .then(res => {
          if (res.data.success === true) {
...
const EmailActions = ({navigation}) => {
  ...
  async function populateData() {
    try {
      ...
      await axios
        .get(`${apiURL}/events/emails/${CURRENT_EVENT_ID}`)
        .then(res => {
          if (res.data.success === true) {
...
export default function EventProfileAnalytics({navigation}) {
          ...
          return (
            <View key={elem.id}>
                      ...
                      <View
                        ...
                        <Text
                          ...
                          {elem.fee == 0
                            ...
                            : `${
                                elem.currency
                                  ? currencySymbolConverter(elem.currency)
                                  : ''
                              } ${elem.fee}`}
                        </Text>
                      </View>
              ...
              <View style={{paddingHorizontal: 10}}>
                <Text style={styles.description}>
                  Event Fee:{' '}
                  {elem.currency ? currencySymbolConverter(elem.currency) : ''}{' '}
                  {elem.fee}
                </Text>
              </View>
              <View style={{paddingHorizontal: 10}}>
